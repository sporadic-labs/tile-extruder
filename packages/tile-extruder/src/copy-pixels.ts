...
function copyPixels({
  ...
  const iterator = srcImage.scanIterator(srcX, srcY, srcW, srcH);
  for (const { x: curSrcX, y: curSrcY, idx: curSrcIndex } of iterator) {
...
/**
 * This copies a single source pixel to every pixel in the destination rectangle
...
function copyPixelToRect({
  ...
  const iterator = destImage.scanIterator(destX, destY, destW, destH);
  for (const { idx: curDestIndex } of iterator) {
